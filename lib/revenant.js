...
async function fetchWatchlist(watchlistAdapter, config, logger) {
  try {
    return await watchlistAdapter.fetch(config);
  } catch (error) {
...
async function runRevenant(argv, dependencies) {
  ...
  program
    ...
    .description("download new torrent files")
    .action(async directory => {
      const config = await configAdapter.readConfig();
      const tempConfig = { ...config };

      if (directory) {
        tempConfig.downloadPath = directory;
      }
  ...
  program
    ...
    .action(async () => {
      const config = await configAdapter.readConfig();
      const watchlist = await fetchWatchlist(watchlistAdapter, config, logger);
