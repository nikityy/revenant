...
var Revenant = function(rutracker, config) {
  ...
  if (!_.has(config, 'username') || !_.has(config, 'password')) {
    throw Error("Second argument should has at least 'username'" +
...
Revenant.prototype.createMD5 = function(results) {
  var staticProps = ['state', 'id', 'category', 'title', 'size'],
      resultsOfStaticProps = results.map(result => _.pick(result, staticProps)),
      resultsByHash = {},
      key;

  resultsOfStaticProps.forEach(result => {
    key = md5(JSON.stringify(result));
    resultsByHash[key] = result; 
  });
...
Revenant.prototype.getNewResults = function(query, results) {
  var oldHashes      = this.state[query],
      hashes         = _.keys(results),
      onlyNewHashes  = hashes.filter(x => {
        return (oldHashes.indexOf(x) === -1);
      }),
      onlyNewResults = _.pick(results, onlyNewHashes);
...
Revenant.prototype.update = function() {
  ...
  async.each(this.config.watch_list, function (query, callback) {
    revenant.rutracker.search(query, function(results) {
      var md5s       = revenant.createMD5(results),
          newResults = revenant.getNewResults(query, md5s),
          newHashes  = _.keys(newResults),
          newValues  = _.values(newResults);

      if (newHashes.length !== 0) {
        hasChanged = true;
        revenant.state[query] = _.keys(md5s);
        revenant.emit('update', newValues);
      }
