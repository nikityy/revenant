...
async function downloadNewTorrents(rutracker, config) {
  const [torrents, newConfig] = await fetchNewTorrents(rutracker, config);
  const newTorrents = [];

  newConfig.watchlist.forEach(query => newTorrents.push(...torrents[query]));

  await Promise.all(
    newTorrents.map(async torrent => {
      const stream = await rutracker.download(torrent.id);
      const filepath = path.resolve(
        newConfig.downloadPath,
        `${torrent.id}.torrent`
      );
