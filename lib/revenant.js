...
var Revenant = function(rutracker, config) {
  ...
  this.on('error', console.trace.bind(console));
  rutracker.login({
...
Revenant.prototype.getNewResults = function(savedState, results) {
...
Revenant.prototype.getUpdatedResults = function(savedState, results) {
  var ids = _.pluck(results, 'id');
  var oldIds = _.keys(savedState);
...
Revenant.prototype.update = function() {
  ...
  async.each(this.config.watch_list, function (query, callback) {
    revenant.rutracker.search(query).then(function(results) {
      var savedState = revenant.state[query];
      var newResults = revenant.getNewResults(savedState, results);
      var updatedResults = revenant.getUpdatedResults(savedState, results);
