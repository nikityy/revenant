const { authenticate, fetchNewWatchlistTorrents, fetchWatchlistTorrents, fetchWatchlist } = require('../revenant');
...
describe('fetchNewWatchlistTorrents', () => {
  const config = {
    torrents: {
      a: ["1", "2"],
      b: ["4", "5"],
  ...
  test('should return torrents list', async () => {
    ...
    const [torrents, newConfig] = await fetchNewWatchlistTorrents(rutracker, config);
...
describe('fetchWatchlistTorrents', () => {
